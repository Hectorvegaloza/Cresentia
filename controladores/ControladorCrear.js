...
import modelocrearcuenta from "../modelos/modelocrearcuenta.js";
    ...
    const ControladorCrearCuenta = {
      crearUsuario: async (solicitud, respuesta) => {
          try {
            ...
            const nuevoUsuario = new modelocrearcuenta({
              nombre,
              correo,
    ...
    leerUsuario: async (solicitud, respuesta) => {
        try {
            const usuarioEncontrado = await modelocrearcuenta.findById(solicitud.params.id) 
    ...
    leerUsuarios: async (solicitud, respuesta) => {
        try {
        const todosLosUsuarios = await modelocrearcuenta.find();
        respuesta.json({
            resultado: "bien",
    ...
    actualizarUsuario: async (solicitud, respuesta) => {
        try {
            const usuarioActualizado = await modelocrearcuenta.findByIdAndUpdate(
                solicitud.params.id,
                solicitud.body
            );
    ...
    eliminarUsuario: async (solicitud, respuesta) => {
        try {
        const usuarioEliminado = await modelocrearcuenta.findByIdAndDelete(solicitud.params.id)
...
export default ControladorCrearCuenta;